from ast import For
import os
from fastapi import APIRouter, File, Form, status, HTTPException,Depends, UploadFile
...
async def add(Title:str=Form(...),desc:str=Form(...),file: UploadFile = File(...),current_user = Depends(get_current_user)):
    ...
    ab = str(uuid.uuid4())
    img_name = FILEPATH + ab + "." + extension
    file_content = await file.read()
    with open(img_name, "wb") as f:
        f.write(file_content)
    await posts.insert_one({"Title":Title,"desc":desc,"photo":img_name,"user_id":current_user["_id"],"user_name":current_user["name"]})
    new1 = await posts.find_one({"Title":Title,"desc":desc,"photo":img_name,"user_id":current_user["_id"]}) 
    new = getPosts(new1)
    return new
...
async def update(id:str,Title:str=Form(...),desc:str=Form(...),file: Optional[UploadFile] = File(None),current_user = Depends(get_current_user)):
    if(current_user==None):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    one = await posts.find_one({"_id":ObjectId(id)})
    out = getPosts(one)
    if out:
        if file !=None:
            ...
            with open(img_name, "wb") as f:
                f.write(file_content)
            await posts.update_one({"_id":ObjectId(id)},{"$set":{"Title":Title,"desc":desc,"photo":img_name,"user_id":current_user["_id"],"user_name":current_user["name"]}})
            one1 = await posts.find_one({"_id":ObjectId(id)})
            out1 = getPosts(one1)
            return(out1)
            
        else:
            posts.update_one({"_id":ObjectId(id)},{"$set":{"Title":Title,"desc":desc,"photo":out["photo"],"user_id":current_user["_id"],"user_name":current_user["name"]}})
            one1 = await posts.find_one({"_id":ObjectId(id)})
            out1 = getPosts(one1)
            return(out1)
    else :
